...
for p in glob.glob(r"C:\Users\LEET\FR13DMAN\PROGRAM\**\*.md", recursive=True):
    b = open(p, "rb").read()
    check("LF-only: " + os.path.relpath(p, r"C:\Users\LEET\FR13DMAN"), b"\r" not in b and not b.startswith(b"\xef\xbb\xbf"))
...
check("single-space separated, no edge spaces", all(d == b" ".join(d.split(b" ")) and not d.startswith(b" ") and not d.endswith(b" ") and b"  " not in d for d in docs))
...
# 5. example manifest parses, is I-JSON-ish, has every required field, keys in JCS order
m = re.search(r"```json\n(.*?)\n```", text, re.S)
check("json block found", m is not None)
man = json.loads(m.group(1))
required = ["manifest_schema", "corpus_id", "origin", "generator", "engine_commit", "seed", "config",
...
check("config in manifest == golden config", json.dumps(man["config"], sort_keys=True, separators=(",", ":")).encode() == jcs)
check("ground_truth.label == mechanism, family == generator.id",
...
check("corpus counts match golden", (man["corpus"]["n_documents"], man["corpus"]["n_tokens"], man["corpus"]["n_types"]) == (2, 4, 2))
check("doc_length matches golden", man["corpus"]["doc_length"] == {"min": 1, "max": 3, "mean": 2.0})
# fidelity rule: every commanded key has a per_statistic verdict; verdict PASS iff all PASS; null achieved => FAIL
...
def verdict(k):
    ...
    if a is None or a["value"] is None:
        return "FAIL"
    c = man["commanded"][k]; t = c["tolerance"]
    if "abs" in t: return "PASS" if abs(a["value"] - c["value"]) <= t["abs"] else "FAIL"
    if "rel" in t: return "PASS" if abs(a["value"] - c["value"]) <= t["rel"] * abs(c["value"]) else "FAIL"
    if t.get("exact"): return "PASS" if a["value"] == c["value"] else "FAIL"
    return "FAIL"
check("per_statistic verdicts follow the D4 rule", all(ps[k] == verdict(k) for k in ps))
check("overall verdict follows the D4 rule", man["fidelity"]["verdict"] == ("PASS" if all(v == "PASS" for v in ps.values()) else "FAIL"))
check("null achieved value carries a reason", all(("reason" in v) == (v["value"] is None) for v in man["achieved"].values()))
def keys_sorted(o):
